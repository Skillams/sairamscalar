...
import com.h2.services.CustomerService                            //terminal ->~testOnly *Length*
...
class LengthAndSizeSpec extends UnitSpec {
 ...
 /* behavior of "CustomerService for size"
  it should "return correct size for number of  customer's  created" in {
    ...
    customerIds: Seq[UUID]= newCustomers.map(newCustomer =>
